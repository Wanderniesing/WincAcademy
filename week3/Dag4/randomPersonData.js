...
const region = randomPersonData.map(region => {
  return region.region;
});
const name = randomPersonData.filter(name => {
  return name.name;
});

const filterKeys = ['gender', 'age'];

const multiFilter = (randomPersonData = [], filterKeys = [], value = '') =>
...
let filteredData = multiFilter(randomPersonData, filterKeys);
...
//   data.filter(
//     data =>
//       data.birthday.dmy.includes('22/12/1990') ||
//       data.birthday.dmy.includes('19/01/2000')
//   );

let birthday = multiFilter(randomPersonData, filterKeys);
...
const LandenLijst = randomPersonData.region;
0;
console.log(Landen);
...
region.forEach(region => {
  ...
  const idContent = document.createTextNode(
    '${Landen.region}Landen has the id: ${Landen.id}'
  );
  id.appendChild(idContent);
  LandenLijst.append(id);
});

info = randomPersonData.filter(x => x.gender == 'female' && x.age > 30);
